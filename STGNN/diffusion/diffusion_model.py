...
from diffusion import Transformer
from tools import default, identity, extract
...
class Diffusion_TS(nn.Module):
    def __init__(
        ...
        self.seq_length = seq_length # 原始和生成的序列长度
        ...
        alphas = 1. - betas # 扩散模型的系数
        alphas_cumprod = torch.cumprod(alphas, dim=0) # 用于计算输入张量沿指定维度的累积乘积。它返回一个与输入张量形状相同的张量，其中每个元素是沿给定维度的累积乘积
        alphas_cumprod_prev = F.pad(alphas_cumprod[:-1], (1, 0), value=1.) # 用于在指定维度上对张量进行填充（padding）。这个函数可以将张量扩展到一个更大的尺寸，并在扩展的部分填充指定的值
    ...
    def predict_noise_from_start(self, x_t, t, x0):
        ...
        ) # 这个公式是 前向公式 q(x_t | x_{t-1}) 反求里面的噪声的公式
    ...
    def model_predictions(self, x, t, clip_x_start=False, padding_masks=None):
        ...
        x_start = self.output(x, t, padding_masks) # 直接从xT预测x0
    ...
    @torch.no_grad() # 在该上下文（context）中的计算都不会被跟踪用于计算梯度
    ...
    def _train_loss(self, x_start, t, target=None, noise=None, padding_masks=None):
        ...
        x = self.q_sample(x_start=x_start, t=t, noise=noise)  # noise sample （前向扩散）
        ...
        if self.use_ff:
            ...
            fourier_loss = self.loss_fn(torch.real(fft1), torch.real(fft2), reduction='none')\
                           + self.loss_fn(torch.imag(fft1), torch.imag(fft2), reduction='none') # 实部和虚部损失
    ...
    def forward(self, x, **kwargs):
        ...
        t = torch.randint(0, self.num_timesteps, (b,), device=device).long() # 对于batch的每个，在总时间步长随即生成一个时间t？针对每个样本随机选择一个时间步t,而不是都加噪声到时间步num_timesteps，是为了更有效地训练模型，(使其能够在不同程度的噪声条件下进行去噪？) 这样相当于在每个时间段都学习了，而不是每个样本都从0-numteimesteps？
    ...
    def fast_sample_infill(self, shape, target, sampling_timesteps, partial_mask=None, clip_denoised=True, model_kwargs=None):
        ...
        # [-1, 0, 1, 2, ..., T-1] when sampling_timesteps == total_timesteps
        times = torch.linspace(-1, total_timesteps - 1, steps=sampling_timesteps + 1) # 这里是快速采样（来自DDIM）steps= 生成的点数

        times = list(reversed(times.int().tolist())) # tolist() 是 PyTorch 张量的一个方法，用于将张量转换为 Python 的标准列表
        time_pairs = list(zip(times[:-1], times[1:]))  # [(T-1, T-2), (T-2, T-3), ..., (1, 0), (0, -1)]
        img = torch.randn(shape, device=device) # 标准正态分布噪音
        # 采用的DDIM的训练方式
        for time, time_next in tqdm(time_pairs, desc='conditional samplitargetng loop time step'):
            time_cond = torch.full((batch,), time, device=device, dtype=torch.long)
            pred_noise, x_start, *_ = self.model_predictions(img, time_cond, clip_x_start=clip_denoised) # 输入随机噪音，直接求出X0,再反求出X0对应的噪声

            if time_next < 0:
                img = x_start
                continue
#----------------------------------- ？？？？？？？？ DDIM 和 IDDPM和Guided-DDPM等采样
            alpha = self.alphas_cumprod[time]
            alpha_next = self.alphas_cumprod[time_next]
            sigma = eta * ((1 - alpha / alpha_next) * (1 - alpha_next) / (1 - alpha)).sqrt() #
            c = (1 - alpha_next - sigma ** 2).sqrt()
            ...
            noise = torch.randn_like(img)
            img = pred_mean + sigma * noise # xt-1|xt 求得xt-1
# -----------------------------------
            # Optimized Conditional Sampling
    ...
    def langevin_fn(
        ...
        input_embs_param = torch.nn.Parameter(sample) #  便于下面求导
        # 这段代码开启了梯度计算并进行了反向传播（loss.backward()），这是典型的训练阶段操作
        with torch.enable_grad():
            for i in range(K):
                ...
                if sigma.mean() == 0:
                    logp_term = coef * ((mean - input_embs_param) ** 2 / 1.).mean(dim=0).sum() # 正太分布概率取log 是公式12的后一部分，|（x-u）^2 对应论文公式12？？？， mean - input_embs_param = pred_mean- （pred_mean + sigma * noise） = - sigma * noise
                    infill_loss = (x_start[partial_mask] - tgt_embs[partial_mask]) ** 2 # 相当于是只有true部分的差值参与梯度计算，会返回一个一维张量（1D tensor），即使 x_start 和 tgt_embs 是多维的。这个行为是因为布尔索引会将满足条件的所有元素“压缩”到一个一维张量中，如果是预测任务则是非预测部分
                    infill_loss = infill_loss.mean(dim=0).sum() # L1
                else:
                    logp_term = coef * ((mean - input_embs_param)**2 / sigma).mean(dim=0).sum() # 正太分布概率取log 一般下来是 （x-u）^2/sigma^2
                    infill_loss = (x_start[partial_mask] - tgt_embs[partial_mask]) ** 2
                    infill_loss = (infill_loss/sigma.mean()).mean(dim=0).sum()
            
                loss = logp_term + infill_loss
                loss.backward()
                optimizer.step() # 更新 xt
        ...
        sample[~partial_mask] = input_embs_param.data[~partial_mask] # 这下是把预测部分替换过去， 伪代码F Relace的上一行
        return sample
