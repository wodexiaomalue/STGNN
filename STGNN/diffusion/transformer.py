...
from diffusion.tools import LearnablePositionalEncoding, Conv_MLP,\
                                                       AdaLayerNorm, Transpose, GELU2, series_decomp, load_norm_Laplacian
...
class TrendBlock(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, in_feat, out_feat, act):
        ...
        self.poly_space = torch.stack([lin_space ** float(p + 1) for p in range(trend_poly)], dim=0) # 形状为 (trend_poly, out_dim)
...
class FourierLayer(nn.Module):
    ...
    def topk_freq(self, x_freq):
        ...
        mesh_a, mesh_b = torch.meshgrid(torch.arange(x_freq.size(0)), torch.arange(x_freq.size(2)), indexing='ij')# indexing='ij' 指定使用 ij 索引约定，即返回的网格张量 mesh_a 对应第一个输入的维度（batch），mesh_b 对应第二个输入的维度（feature）
        index_tuple = (mesh_a.unsqueeze(1), indices, mesh_b.unsqueeze(1))
        x_freq = x_freq[index_tuple] # 取出前Topk，高级索引
...
class FullAttention(nn.Module):
    def __init__(self,
        ...
        self.causalquery = nn.Conv1d(in_channels=n_embd, out_channels=n_embd, kernel_size=self.kernel_size,
                                   padding=self.padding)
        self.key = nn.Linear(n_embd, n_embd) # 64,64
    ...
    def forward(self, x, mask=None):
        B, T, C = x.size() # B, 24, 64
        k = self.key(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs) 64,4,24,16
        # q = self.query(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        # self.causalkey(queries.permute(0, 2, 1))[:, :, :-self.padding].permute(0, 2, 1).view(B, L, H, -1)
        # k = self.causalkey(x.permute(0, 2, 1))[:, :, :-self.padding].permute(0, 2, 1).view(B, T, self.n_head,
        #                                                                                    C // self.n_head).transpose(
        #     1, 2)
        q = self.causalquery(x.permute(0, 2, 1))[:, :, :-self.padding].permute(0, 2, 1).view(B, T, self.n_head,
                                                                                           C // self.n_head).transpose(
            1, 2)
...
class FeatureAttention(nn.Module):
    def __init__(self,
        ...
        self.resid_drop = nn.Dropout(resid_pdrop)
        # output projection
        self.proj = nn.Linear(n_embd, max_len)
        self.n_head = n_head
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self,
        ...
        norm_adj, N_dim, max_len = args
        self.gcn_forward = GCN(max_len, n_embd, norm_adj, resid_pdrop=resid_pdrop)

        self.s_emb = Conv_MLP(max_len, n_embd, resid_pdrop=resid_pdrop)
        self.t_emb = Conv_MLP(N_dim, n_embd, resid_pdrop=resid_pdrop)
        ...
        self.linear2 = nn.Linear(n_embd, n_feat)

    def forward(self, input, timestep, pos_dec=None ,mask=None, label_emb=None):
        x = self.s_emb(input.transpose(1, 2))
        if pos_dec != None:
            x = pos_dec(x)
        x = self.gcn_forward(x)
        a, att = self.attn1(self.ln1(x, timestep, label_emb), mask=mask)
        x = input + a.transpose(1, 2)

        x = self.t_emb(x)

        a, att = self.attn2(self.ln1_1(x, timestep), mask=mask)
        x = x + a
        x1, x2 = self.proj(x).chunk(2, dim=1)
        trend, season = self.trend(x1), self.seasonal(x2)
        x = x + self.mlp(self.ln2(x))
        m = torch.mean(x, dim=1, keepdim=True)
        return self.linear2(x - m), self.linear(m), trend, season
...
class Decoder(nn.Module):
    ...
    def forward(self, x, t, pos_dec=None, padding_masks=None, label_emb=None):
        b, c, _ = x.shape
        # att_weights = []
        mean = []
        ...
        for block_idx in range(len(self.blocks)):
            ...
            x, residual_mean, residual_trend, residual_season = \
                self.blocks[block_idx](x,  t, pos_dec=pos_dec, mask=padding_masks, label_emb=label_emb)
...
class GCN(nn.Module):
    def __init__(self, seq_len, n_embd, norm_adj, resid_pdrop=0.):
        super().__init__()
        self.register_buffer('norm_adj', norm_adj.float())
        self.project = nn.Sequential(nn.Linear(n_embd, n_embd), nn.GELU(), nn.Dropout(p=resid_pdrop))

    def forward(self, x):
        # x: # B，N, D
        x = torch.matmul(self.norm_adj, x)
        x = self.project(x)
        return x
...
class Transformer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        if not kwargs.get('adj_path', ""):
            raise ValueError("The 'adj_path' parameter cannot be an empty string.")
        norm_adj, N_dim = load_norm_Laplacian(kwargs.get('adj_path', ""), "cuda" if torch.cuda.is_available() else "cpu")


        self.inverse = Conv_MLP(n_feat, n_feat, resid_pdrop=resid_pdrop)
        ...
        self.pos_dec = LearnablePositionalEncoding(n_embd, dropout=resid_pdrop, max_len=N_dim)
        self.decoder = Decoder(n_channel, n_feat, n_embd, n_heads, n_layer_dec, attn_pdrop, resid_pdrop, mlp_hidden_times,
                               block_activate, *(norm_adj, N_dim, max_len))

    def forward(self, input, t, padding_masks=None, return_res=False):

        output, mean, trend, season = self.decoder(input, t, pos_dec = self.pos_dec , padding_masks=padding_masks)
