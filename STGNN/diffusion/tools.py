...
class SinusoidalPosEmb(nn.Module):
    ...
    def forward(self, x):
        ...
        emb = x[:, None] * emb[None, :] # 这里 x[:, None] 将输入 x 的形状扩展为 (batch_size, 1)， emb[None, :] 将 emb 扩展为 (1, half_dim)。、
...
def load_norm_Laplacian(adj_path, device, dtype=np.float32):
    ...
    D = np.diag(np.sum(adj_np, axis=1)+ 1e-6)
    scaling_factor = 0.5
    sym_norm_Adj_matrix = np.dot(np.sqrt(D)* scaling_factor, adj_np)
    sym_norm_Adj_matrix = np.dot(sym_norm_Adj_matrix, np.sqrt(D)* scaling_factor)
